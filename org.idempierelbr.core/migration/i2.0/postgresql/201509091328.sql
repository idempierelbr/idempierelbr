...
SELECT 
 CASE 
 /* detecta atraso */
 WHEN daysBetween( $2 , NextBusinessDay(COALESCE(SubtractDays(bd.LBR_InterestDate,1),b.DueDate),b.ad_client_id) ) > 0 THEN 
    CASE
    /* taxa mensal */
    ...
    * daysBetween( $2 , NextBusinessDay(COALESCE(SubtractDays(bd.LBR_InterestDate,1),b.DueDate),b.ad_client_id) )
 ELSE 0
 END interestvalue
...
SELECT 
 CASE 
 /* detecta atraso */
 WHEN daysBetween( $2 , NextBusinessDay(COALESCE(SubtractDays(bd.LBR_LatePaymentPenaltyDate,1),b.dueDate),b.ad_client_id) ) > 0 THEN 
    CASE
    /* taxa percentual */
