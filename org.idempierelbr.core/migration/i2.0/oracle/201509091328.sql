...
BEGIN
 ...
 CASE 
 /* detecta atraso */
 WHEN daysBetween( p_PaymentDate , NextBusinessDay(COALESCE(SubtractDays(bd.LBR_InterestDate,1),b.DueDate),b.ad_client_id) ) > 0 THEN 
    CASE
    /* taxa mensal */
    ...
    * daysBetween( p_PaymentDate , NextBusinessDay(COALESCE(SubtractDays(bd.LBR_InterestDate,1),b.DueDate),b.ad_client_id) )
 ELSE 0
 END
...
SELECT 
 CASE 
 /* detecta atraso */
 WHEN daysBetween( p_PaymentDate , NextBusinessDay(COALESCE(SubtractDays(bd.LBR_LatePaymentPenaltyDate,1),b.dueDate),b.ad_client_id) ) > 0 THEN 
    CASE
    /* taxa percentual */
...
SELECT lbr_register_migration_script('201509091328.sql') FROM dual;
