...
package org.idempierelbr.core.webui.editor;
...
import org.idempierelbr.core.webui.window.WLocationDialogLBR;
 ...
 **/
public class WLocationEditorLBR extends WEditor implements EventListener<Event>, PropertyChangeListener, ContextMenuListener
{
    private static final String[] LISTENER_EVENTS = {Events.ON_CLICK};
    
    private static CLogger log = CLogger.getCLogger(WLocationEditorLBR.class);
    ...
    **/
    public WLocationEditorLBR(String columnName, boolean mandatory, boolean isReadOnly, boolean isUpdateable,
     ...
     * @param gridField
     */
    public WLocationEditorLBR(GridField gridField) {
    ...
    {    
        ...
        if ("onClick".equals(event.getName()))
        {
            ...
            final WLocationDialogLBR ld = new WLocationDialogLBR(Msg.getMsg(Env.getCtx(), "Location"), m_value, gridField);
            ld.addEventListener(DialogEvents.ON_WINDOW_CLOSE, new EventListener<Event>() {
				...
				public void onEvent(Event event) throws Exception {
		            ...
		            ValueChangeEvent valuechange = new ValueChangeEvent(WLocationEditorLBR.this,getColumnName(),null,null);
		            fireValueChange(valuechange);   //  resets m_mLocation
		            if (C_Location_ID != 0)
		            {
		                ValueChangeEvent vc = new ValueChangeEvent(WLocationEditorLBR.this,getColumnName(),null,ii);
		                fireValueChange(vc);
		            }
		            setValue(ii);					
				}
