...
import org.idempierelbr.base.model.MLBRBoletoStaticData;
 ...
 *	Generate CNAB
 */
public class FebrabanGenerate extends SvrProcess
{
	/**	Manual Selection		*/
	...
	private HeaderArquivo generateHeaderArquivo() {
		...
		for(Banco banco : Banco.values()) {
			if (banco.getCodigo() == Integer.parseInt(m_bank.getRoutingNo()))
				header.setNomeBanco(banco.getDescricao());
		}	
		...
		// Layout do Arquivo
		arquivo.setNumeroVersaoLayout(88);
		
		// Densidade
		arquivo.setDensidadeGravacaoArquivo(1600);
	...
	private HeaderTitulosEmCobranca generateHeaderTitulosEmCobranca() {
		...
		servico.setNumeroVersaoLayoutLote(46);
		...
		//-------------------------------Data do Crédito----------------------------------
		header.setDataCredito(m_LBR_FileGeneratingDate);
	...
	private Empresa getEmpresa(int tamanhoNumInscricao) {
		...
		empresa.setCodigoConvenio(convenio.getLBR_ConvenioNo());
