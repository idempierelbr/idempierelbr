 ...
 * @since 1.0.0
 * 
 * Added the ability to create new records.
 * by @contributor Claudemir Todo Bom <claudemir@todobom.com>
 * 
 */
...
public class LBRFixedFormatManager implements FixedFormatManager {
  ...
  public <T> T load(Class<T> fixedFormatRecordClass) {
	    HashMap<String, Object> foundData = new HashMap<String, Object>();
	    HashMap<String, Class<?>> methodClass = new HashMap<String, Class<?>>();
	    //assert the record is marked with a Record
	    getAndAssertRecordAnnotation(fixedFormatRecordClass);
	    ...
	    for (Method method : allMethods) {
	      ...
	      Fields fieldsAnnotation = method.getAnnotation(Fields.class);
	      if (fieldAnnotation != null) {
	        initializeFieldData(fixedFormatRecordClass, instance , foundData, methodClass, method, methodName, fieldAnnotation);
	      } else if (fieldsAnnotation != null) {
	        //assert that the fields annotation contains minimum one field anno
	        if (fieldsAnnotation.value() == null || fieldsAnnotation.value().length == 0) {
	          throw new FixedFormatException(format("%s annotation must contain minimum one %s annotation", Fields.class.getName(), Field.class.getName()));
	        }
	        initializeFieldData(fixedFormatRecordClass, instance , foundData, methodClass, method, methodName, fieldsAnnotation.value()[0]);
	      }
	    }
  ...
  private <T> void initializeFieldData(Class<T> fixedFormatRecordClass, T instance , HashMap<String, Object> foundData, HashMap<String, Class<?>> methodClass, Method method, String methodName, Field fieldAnnotation) {
