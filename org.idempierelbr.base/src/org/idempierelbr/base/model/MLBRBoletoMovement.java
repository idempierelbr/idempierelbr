...
public class MLBRBoletoMovement extends X_LBR_BoletoMovement {
	...
	public String createPayment() {
		...
		if ( (invoice != null) && (invoice.getDocStatus()).equals("CO")){
			if (!invoice.isPaid()){
				...
				if (boleto.getC_InvoicePaySchedule_ID() > 0)
	...
	public void detectBankAccount(int agenciaCedente, String agencyDV, long contaCedente, String accountDV, String routingNo) {
		String agencyWithDV = String.format( "%d-%s", agenciaCedente, agencyDV!=null?agencyDV:"0" );
		String agency = String.format("%d", agenciaCedente);

		String accountWithDV = String.format( "%d-%s", contaCedente, accountDV!=null?accountDV:"0" );
		String account = String.format("%d", contaCedente);
