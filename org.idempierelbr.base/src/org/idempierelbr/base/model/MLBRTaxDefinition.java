...
public class MLBRTaxDefinition extends X_LBR_TaxDefinition
{
	...
	public static MLBRTaxDefinition[] get (int AD_Client_ID, int AD_Org_ID, int C_BPartner_ID, int C_DocType_ID, 
			int C_Region_ID, int To_Region_ID, int C_City_ID, int LBR_To_City_ID,
			int LBR_BPartnerCategory_ID, int LBR_FiscalGroup_BPartner_ID,
			...
			boolean isSOTrx, String lbr_TransactionType, Timestamp validFrom)
	{
		...
		where += "AND (C_City_ID IS NULL OR C_City_ID=?) ";
		where += "AND (LBR_To_City_ID IS NULL OR LBR_To_City_ID=?) ";
		...
		List<MLBRTaxDefinition> list = new Query (Env.getCtx(), MLBRTaxDefinition.Table_Name, where, null)
			.setParameters(new Object[]{AD_Client_ID, AD_Org_ID, C_BPartner_ID, C_DocType_ID,
					C_Region_ID, To_Region_ID, C_City_ID, LBR_To_City_ID, 
