...
public class MLBRIBPTax extends X_LBR_IBPTax {
	 ...
	 * @return
	 */
	public static BigDecimal getTaxAmt(Properties ctx, Integer LBR_NCM_ID, String productSource, BigDecimal sourceAmt,
			String trxName) {
		...
		if (m_ibptax != null && sourceAmt != null && sourceAmt.signum() != 0) {
			...
			// product source
			if (productSource.equals("0") || productSource.equals("3") || productSource.equals("4")
					|| productSource.equals("5"))
			...
			// multiply sourceAmt by total tax rate to get the grand total of
			// all taxes
	...
	public static String getIBPTaxDescription(PO po, boolean IsSummary) {
		...
		String sql = " SELECT 								"
				...
				+ "	COALESCE(t.LBR_ImportedFederalTax, 0)  AS BaseCalcAmt,				"
		...
		try {
			...
			if (IsSummary) {
				...
				// only total tax
				message += "Tributos Totais Incidentes (Lei Federal 12.741/2012): R$ " + TextUtil.bigdecimalToString(totalTaxAmt);
			...
			log.severe("Não foi possível gerar descrição de impostos aproximados. Erro: " + po);
			e.printStackTrace();
