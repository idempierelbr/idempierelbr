...
public class MLBRNotaFiscalEventLine extends X_LBR_NotaFiscalEventLine {
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		if (getLBR_NFeXML_ID() > 0) {
			// Check consistency with Lot Org
			MLBRNFeXML nfeXML = new MLBRNFeXML(getCtx(), getLBR_NFeXML_ID(), get_TrxName());

			if (nfeXML.getAD_Org_ID() != getAD_Org_ID()) {
				log.saveError("Error", Msg.parseTranslation(getCtx(), "NF-e XML org. is different than Lot org."));
				return false;
			}
			
			// Fill in NF-e ID
			setLBR_NFeID(nfeXML.getLBR_NFeID());
		}
